...
export default function MyPerformanceScreen() {
  ...
  // Prepare Line Chart Data - FIXED: Use simple labels
  ...
  // Prepare Pie Chart Data - FIXED: Handle all materials properly
  const preparePieChartData = () => {
    ...
    // Filter out materials with 0 points
...
population: Number(item.total_points),
  ...
  // Check if we have any data for pie chart
  ...
  <View style={styles.achievementRow}>
    ...
    <Text style={styles.achievementText}>
      ...
      <View style={styles.debugCard}>
        ...
        <Text>Filter: {timeFilter} | {materialFilter}</Text>
        <Text>Data from: {apiConnected ? 'Database' : 'Sample'}</Text>
        <Text>Items: {performanceData.summary.total_items}</Text>
        <Text>Points: {performanceData.summary.total_points}</Text>
      </View>
    </ScrollView>
  );
}
