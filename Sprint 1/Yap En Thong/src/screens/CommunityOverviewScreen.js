...
// Colors for charts
const facultyColors = {
  'FKE': '#4CAF50',     // Engineering - Green
  'FS': '#2196F3',      // Science - Blue
  'FABU': '#FF9800',    // Built Environment - Orange
  'FKT': '#9C27B0',     // Technology - Purple
  'FK': '#607D8B',      // Computing - Blue Gray
  'FKM': '#795548',     // Management - Brown
  'FSSH': '#009688',    // Social Sciences - Teal
  'FEST': '#F44336',    // Education - Red
  'FM': '#3F51B5',      // Medicine - Indigo
  'SPACE': '#E91E63'    // Space - Pink
};
...
const loadCommunityData = async () => {
  ...
  /*const loadCommunityData = async () => {
    ...
    try {
      ...
      const result = await apiService.getCommunityOverview(filters);
      if (result && result.success && result.data) {
        console.log('Community data loaded:', result.data);
        setCommunityData(result.data);
      } else {
        // Fallback sample data based on filters
        setCommunityData(getSampleData(filters.faculty, filters.semester));
      }
    } catch (error) {
  ...
  // Enhanced sample data generator based on filters
  const getSampleData = (faculty, semester) => {
    ...
    // If specific faculty selected, filter data
