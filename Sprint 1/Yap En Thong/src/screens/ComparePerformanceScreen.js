...
// Filter options - REMOVED "All Time"
...
export default function ComparePerformanceScreen() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Prepare chart data immediately
  ...
  const prepareChartData = (data) => {
    if (!data || !data.trendData) {
      // Default data when no trend data available
      setChartData({
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 30 }}>
      ...
      {__DEV__ && (
        <View style={styles.debugCard}>
          ...
          <Text>Your Points: {compareData.performanceSummary.yourPoints}</Text>
          <Text>Points to Next: {compareData.pointsToNext}</Text>
          <Text>Chart Data: {chartData?.labels?.join(', ')}</Text>
        </View>
      )}
    </ScrollView>
  );
