...
// ------------------- CONTRIBUTIONS -------------------
app.post('/contribution/add', (req, res) => {
    let { studentID, eventID, binID, pointsEarned, itemsScanned } = req.body;
    ...
    if (studentID == null || eventID == null || binID == null || !Array.isArray(itemsScanned))
        return res.status(400).json({ message: 'All fields required' });

    const itemsJSON = JSON.stringify(itemsScanned);

    db.query(
        'SELECT * FROM participation WHERE studentID=? AND eventID=? AND participationStatus IN ("Registered","Attended","Completed")',
        [studentID, eventID],
        (err, participationResults) => {
            if (err) return res.status(500).json({ message: 'DB error checking participation' });
            if (participationResults.length === 0)
                return res.status(400).json({ message: 'Student not registered for this event' });

            db.query(
                'INSERT INTO contribution (studentID, eventID, binID, pointsEarned, itemsScanned, contributionDate) VALUES (?, ?, ?, ?, ?, NOW())',
                [studentID, eventID, binID, pointsEarned, itemsJSON],
                (err2) => {
                    if (err2) return res.status(500).json({ message: 'Failed to add contribution' });

                    db.query(
                        'UPDATE participation SET rewardPointsEarned = rewardPointsEarned + ? WHERE studentID=? AND eventID=?',
                        [pointsEarned, studentID, eventID],
                        (err3) => {
                            if (err3) return res.status(500).json({ message: 'Failed to update participation points' });
                            return res.json({ message: 'Contribution added successfully' });
                        }
                    );
                }
            );
...
// ------------------- RECYCLING BINS -------------------
// ------------------- RECYCLING BINS (Hardcoded) -------------------
app.get('/recyclingbins', (req, res) => {
    const bins = [
