  ...
  static async debugFacultyData(faculty) {
    const debugQuery = `
      SELECT 
        ...
        COUNT(DISTINCT rt.user_id) AS students_with_transactions,
  ...
  static async getCurrentSemesterEngagement() {
    const query = `
      SELECT 
        ...
        ROUND((COUNT(DISTINCT rt.user_id) / COUNT(DISTINCT s.studentID)) * 100, 2) as participation_rate_percent,
        COALESCE(SUM(rt.points_earned), 0) as total_points,
        COALESCE(COUNT(rt.id), 0) as total_transactions,
        ROUND(COALESCE(SUM(rt.points_earned) / NULLIF(COUNT(DISTINCT rt.user_id), 0), 0), 2) as avg_points_per_active_student,
        ROUND(COALESCE(COUNT(rt.id) / NULLIF(COUNT(DISTINCT rt.user_id), 0), 0), 2) as avg_transactions_per_active_student
      FROM Student s
      LEFT JOIN User u ON s.userID = u.userID
      LEFT JOIN recycling_transactions rt ON u.userID = rt.user_id 
    ...
    const query = `
      WITH EngagementData AS (
        SELECT 
          ...
          COUNT(DISTINCT s.studentID) as total_students,
          COUNT(DISTINCT rt.user_id) as active_recyclers,
          ROUND((COUNT(DISTINCT rt.user_id) / COUNT(DISTINCT s.studentID)) * 100, 2) as participation_rate,
          COALESCE(AVG(rt.points_earned), 0) as avg_points_per_active_student,
          COALESCE(COUNT(rt.id) / NULLIF(COUNT(DISTINCT rt.user_id), 0), 0) as avg_transactions_per_active_student
        FROM Student s
        LEFT JOIN User u ON s.userID = u.userID
        LEFT JOIN recycling_transactions rt ON u.userID = rt.user_id 
          AND rt.transaction_date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
    ...
    const query = `
      WITH MonthlyEngagement AS (
        SELECT 
          ...
          ROUND(COALESCE(COUNT(DISTINCT rt.user_id) / NULLIF(COUNT(DISTINCT s.studentID), 0), 0) * 100, 2) as monthly_participation_rate,
  ...
  static async getCampusZoneEngagement() {
    const query = `
      SELECT 
        ...
        COUNT(DISTINCT s.studentID) as total_students,
        COUNT(DISTINCT rt.user_id) as active_recyclers,
        ROUND(COALESCE(COUNT(DISTINCT rt.user_id) / NULLIF(COUNT(DISTINCT s.studentID), 0), 0) * 100, 2) as zone_participation_rate,
    ...
    try {
      ...
      const createViewQuery = `
        ...
        SELECT 
          ...
          COUNT(DISTINCT s.studentID) as total_students,
          COUNT(DISTINCT rt.user_id) as active_recyclers,
          ROUND(COALESCE(COUNT(DISTINCT rt.user_id) * 100.0 / NULLIF(COUNT(DISTINCT s.studentID), 0), 0), 2) as participation_rate_percent,
          COALESCE(SUM(rt.points_earned), 0) as total_points,
      ...
      const query = `
        ...
        WITH FacultyPerformance AS (
          SELECT 
            ...
            COUNT(DISTINCT rt.user_id) as active_recyclers,
            ROUND(
              COALESCE(COUNT(DISTINCT rt.user_id) * 100.0 / NULLIF(COUNT(DISTINCT s.studentID), 0), 0), 
              2
  ...
  static async getExecutiveDashboardMetrics() {
    const query = `
      ...
      SELECT 
        ...
        CONCAT(
          ROUND((SELECT COUNT(DISTINCT rt.user_id) 
                 FROM recycling_transactions rt 
        ...
        (SELECT COUNT(*) 
         FROM (
           ...
           LEFT JOIN recycling_transactions rt ON u.userID = rt.user_id 
             AND rt.transaction_date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
           GROUP BY s.faculty
           HAVING COUNT(DISTINCT rt.user_id) * 100.0 / COUNT(DISTINCT s.studentID) < 40
         ) as low_engagement
        ) as value,
        CASE 
          WHEN (SELECT COUNT(*) 
    ...
    const query = `
      SELECT 
        ...
        (SELECT s.faculty 
         FROM Student s
         JOIN User u ON s.userID = u.userID
         JOIN recycling_transactions rt ON u.userID = rt.user_id
         GROUP BY s.faculty
         ORDER BY COUNT(DISTINCT rt.user_id) * 100.0 / COUNT(DISTINCT s.studentID) DESC 
         LIMIT 1) as value,
                ...
                FROM (
                  ...
                  FROM (
                    SELECT 
                      s.faculty,
                      DATE_FORMAT(rt.transaction_date, '%Y-%m') as month_year,
                      COUNT(DISTINCT rt.user_id) * 100.0 / COUNT(DISTINCT s.studentID) as monthly_participation_rate
                    FROM Student s
                    JOIN User u ON s.userID = u.userID
                    LEFT JOIN recycling_transactions rt ON u.userID = rt.user_id
