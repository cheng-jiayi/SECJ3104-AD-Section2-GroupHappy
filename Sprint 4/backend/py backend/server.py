...
def register_user():
    ...
    try:
        ...
        # Extract all fields - handle both 'fullName' and 'name' for compatibility
        full_name = data.get('fullName', '').strip()
        if not full_name:
            full_name = data.get('name', '').strip()  # Fallback to 'name'
            
        username = data.get('username', '').strip()
        password = data.get('password', '').strip()
        role = data.get('role', '').strip().lower()
        email = data.get('email', '').strip()
        utm_id = data.get('utmID', '').strip()
        ...
        logger.info(f"Extracted faculty_input: '{faculty_input}'")
        logger.info(f"Extracted utm_id: '{utm_id}'")
        logger.info(f"Registration attempt: {username}, role: {role}, matric: {matric_no}")
        
        # ============ FIX 1: Extract faculty code from input ============
        def extract_faculty_code(faculty_input):
            """Extract faculty code from input (handles both full name and code)"""
            if not faculty_input:
                return ''
            
            # Clean the input
            faculty_clean = faculty_input.strip().upper()
            
            # If it contains parentheses, extract the code before '('
            if '(' in faculty_clean:
                # Format like "FABU (Faculty of Built Environment & Surveying)"
                code = faculty_clean.split('(')[0].strip()
                return code
            
            # If it's multiple words, take the first word (should be the code)
            if ' ' in faculty_clean:
                code = faculty_clean.split(' ')[0].strip()
                # Check if it's a valid 2-4 letter code
                if len(code) >= 2 and len(code) <= 4:
                    return code
            
            # Otherwise return the cleaned input
            return faculty_clean
        
        # Extract faculty code
        faculty = extract_faculty_code(faculty_input)
        logger.info(f"Extracted faculty code: '{faculty}' from input: '{faculty_input}'")
        
        # ============ END OF FIX 1 ============
        
        # ============ FIX 2: Validate UTM ID - NEW RULES ============
        # Accept any format including just numbers or special characters
        # But ensure it's not empty and not just whitespace
        if not utm_id or utm_id.strip() == '':
            logger.error("❌ UTM ID is empty")
            return jsonify({
                'success': False,
                'error': 'UTM ID is required'
            }), 400
        ...
        if len(utm_id) < 2:
            logger.error(f"❌ UTM ID too short: '{utm_id}'")
            return jsonify({
                'success': False,
                'error': 'UTM ID must be at least 2 characters'
            }), 400
        
        # Check maximum length (adjust based on your database column size)
        if len(utm_id) > 20:
            logger.error(f"❌ UTM ID too long: '{utm_id}'")
            return jsonify({
                'success': False,
                'error': 'UTM ID must be less than 20 characters'
            }), 400
        
        # Check for invalid characters (allow letters, numbers, hyphens, underscores)
        import re
        utm_regex = r'^[A-Za-z0-9\-_]+$'
        if not re.match(utm_regex, utm_id):
            logger.error(f"❌ Invalid UTM ID format: '{utm_id}'")
            return jsonify({
                'success': False,
                'error': 'UTM ID can only contain letters, numbers, hyphens (-), and underscores (_)'
            }), 400
        
        # Check if UTM ID is just a single dash or problematic value
        if utm_id == '-' or utm_id == '_':
            logger.error(f"❌ Invalid UTM ID value: '{utm_id}'")
            return jsonify({
                'success': False,
                'error': 'UTM ID cannot be just a single dash or underscore'
            }), 400
        
        # Convert to uppercase for consistency (optional)
        utm_id = utm_id.upper()
        logger.info(f"✅ Validated UTM ID: '{utm_id}'")
        # ============ END OF FIX 2 ============
        ...
        required_fields = {
            ...
            'utmID': utm_id,
            'matricNo': matric_no,
            'faculty': faculty  # Use extracted faculty code, not original input
        }
        
        # Debug each field
        logger.info("=== VALIDATING REQUIRED FIELDS ===")
        for field, value in required_fields.items():
            logger.info(f"{field}: '{value}' (empty: {not value})")
        ...
        # Matric number validation (A23CS0001 format)
        matric_regex = r'^A\d{2}[A-Z]{2}\d{4}$'
        if not re.match(matric_regex, matric_no.upper()):
            return jsonify({
                'success': False,
                'error': 'Matric number must be in format A23CS0001'
            }), 400
        
        # Faculty validation - MUST match database ENUM values
        valid_faculties = ['FABU', 'FS', 'FKT', 'FKE', 'FK', 'FKM', 'FSSH', 'FEST', 'FM', 'SPACE']
        if faculty.upper() not in valid_faculties:
            logger.error(f"❌ Invalid faculty code: '{faculty}'")
            logger.error(f"Valid faculty codes are: {valid_faculties}")
            logger.error(f"Original input was: '{faculty_input}'")
            return jsonify({
                'success': False,
                'error': f'Invalid faculty code: {faculty}. Must be one of: {", ".join(valid_faculties)}'
            }), 400
        
        # Ensure year_of_study is integer
        try:
            if isinstance(year_of_study, str):
                year_of_study = int(year_of_study)
            if year_of_study < 1 or year_of_study > 5:
                year_of_study = 1  # Default to 1 if out of range
        except ValueError:
            year_of_study = 1
        ...
        # ============ FIX 3: Check for duplicate entries ============
        ...
        # Check if UTM ID already exists - THIS IS THE MAIN FIX
        cursor.execute("SELECT userID FROM User WHERE utmID = %s", (utm_id,))
        existing_utm = cursor.fetchone()
        
        if existing_utm:
            logger.warning(f"UTM ID already registered: {utm_id}")
            return jsonify({
                'success': False,
                'error': 'UTM ID already registered. Please use a different UTM ID.'
            }), 400
        
        # Check if matric number already exists (for students only)
        if role == 'student':
            cursor.execute("SELECT studentID FROM Student WHERE studentID = %s", (matric_no.upper(),))
            existing_matric = cursor.fetchone()
            
            if existing_matric:
                logger.warning(f"Matric number already registered: {matric_no}")
                return jsonify({
                    'success': False,
        ...
        # Generate userID
        user_id = f"U{datetime.now().strftime('%Y%m%d%H%M%S')}{random.randint(100, 999)}"
        logger.info(f"Generated userID: {user_id}")
        ...
        try:
            cursor.execute("""
                INSERT INTO User 
                (userID, username, password, fullName, utmID, email, role, 
                 contactNumber, address, profilePicture, defaultPassword, createdDateTime)
            ...
            logger.error(f"❌ Error inserting into User table: {insert_error}")
            # Check specifically for duplicate UTM ID error
            if "Duplicate entry" in str(insert_error) and "utmID" in str(insert_error):
                logger.error(f"❌ UTM ID duplicate (database level): {utm_id}")
                return jsonify({
                    'success': False,
                    'error': 'UTM ID already registered. Please use a different UTM ID.'
                }), 400
        ...
        if role == 'student':
            logger.info("Inserting into Student table...")
            logger.info(f"Student data: ID={matric_no.upper()}, Faculty={faculty.upper()}, Year={year_of_study}")
            
            try:
                cursor.execute("""
                    INSERT INTO Student 
                    (studentID, userID, faculty, yearOfStudy, 
                     totalPoints, totalMerits, totalItemsRecycled, totalWeightRecycled)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """, (
                    matric_no.upper(),  # Ensure uppercase
                    user_id,
                    faculty.upper(),    # Use extracted faculty code in uppercase
                    int(year_of_study),
                    0,  # totalPoints
                    0,  # totalMerits
                ...
                logger.info(f"✅ Student registered: {matric_no.upper()}, Faculty: {faculty.upper()}, Year: {year_of_study}")
            except Error as student_error:
                logger.error(f"❌ Error inserting into Student table: {student_error}")
                logger.error(f"SQL that failed: INSERT INTO Student ...")
                logger.error(f"Values: ({matric_no.upper()}, {user_id}, {faculty.upper()}, {year_of_study}, ...)")
                raise
        
        # If role is admin, insert into Admin table
        elif role == 'admin':
            admin_id = f"A{datetime.now().strftime('%Y%m%d%H%M%S')}{random.randint(100, 999)}"
            ...
            logger.info(f"✅ Admin registered: {admin_id}")
        ...
        return jsonify({
            ...
            'fullName': full_name,
            'email': email,
            'studentID': matric_no.upper() if role == 'student' else None,
        ...
        if "Duplicate entry" in error_message:
            ...
            elif "studentID" in error_message:
                return jsonify({
                    'success': False,
                    'error': 'Matric number already registered'
                }), 400
            ...
            elif "utmID" in error_message:
                return jsonify({
                    'success': False,
                    'error': 'UTM ID already registered. Please use a different UTM ID.'
                }), 400
        
        # Handle faculty ENUM error
        if "Data truncated for column 'faculty'" in error_message or "Incorrect enum value" in error_message:
            return jsonify({
                'success': False,
                'error': f'Invalid faculty code: {faculty if "faculty" in locals() else "unknown"}. Must be one of: {", ".join(valid_faculties)}'
            }), 400
        ...
        if "foreign key constraint fails" in error_message:
            return jsonify({
                'success': False,
                'error': 'Database constraint error. Please contact administrator.'
            }), 400
...
@app.route('/api/students/<user_id>/merit-info', methods=['GET'])
def get_student_merit_info(user_id):
    try:
        cursor = db.cursor(dictionary=True)
        
        # Get student's merit points and completed events
        cursor.execute("""
            SELECT 
                s.studentID,
                s.totalPoints,
                ...
                (SELECT COUNT(*) FROM Participation p 
                 ...
                 AND p.participationStatus = 'Completed') as completed_events
            FROM Student s
            JOIN User u ON s.userID = u.userID
            WHERE u.userID = %s
        """, (user_id,))
        ...
        return jsonify({
            ...
            'totalPoints': student['totalPoints'] or 0,
            'totalMerits': student['totalMerits'] or 0,
            'totalItemsRecycled': student['totalItemsRecycled'] or 0,
            'totalWeightRecycled': float(student['totalWeightRecycled']) if student['totalWeightRecycled'] else 0.0,
            'completedEvents': student['completed_events'] or 0
        })
        
    except Exception as e:
        print(f"Error fetching merit info: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def get_server_info():
    ...
    return jsonify({
        ...
        'connection_methods': {
            'usb_debugging': 'Use: http://10.0.2.2:5000',
            ...
            if user['role'] == 'student':
                try:
                    ...
                    if rows_affected > 0:
                        ...
                        cursor.execute("""
                            SELECT totalPoints, totalItemsRecycled, totalWeightRecycled
                            FROM Student 
                            WHERE studentID = %s
                        """, (student_id,))
...
if __name__ == '__main__':
    ...
    print("\n🔗 CONNECTION METHODS:")
    print("   For USB Debugging: http://10.0.2.2:5000")
    ...
    print("   4. In React Native app, use: http://10.0.2.2:5000")
    ...
    print("\n" + "=" * 60)
    print("💡 TIP: Test server with: curl http://10.0.2.2:5000/api/test")
    print("=" * 60 + "\n")
