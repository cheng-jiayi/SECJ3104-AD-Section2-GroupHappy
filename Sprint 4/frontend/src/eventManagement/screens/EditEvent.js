...
// Helper function to format the image URL (moved outside component)
const getEventImageUrl = (imageUrl) => {
  ...
  // If it's just a filename, construct with /uploads/ prefix
  return `http://10.0.2.2:3000/uploads/${imageUrl}`;
};

export default function EditEvent({ route, navigation }) {
  const { event } = route.params;
  
  // Debug the event data to see what we're working with
  useEffect(() => {
    console.log('📦 Event data received:', {
      id: event.eventID,
      title: event.eventTitle,
      imageURL: event.eventImageURL,
      hasImage: !!event.eventImageURL
    });
    
    // Get the formatted image URL
    const currentImageUrl = getEventImageUrl(event.eventImageURL);
    console.log('🔗 Formatted image URL:', currentImageUrl);
  }, [event]);
  ...
  // ✅ CORRECT: Helper function, not JSX
  const getCurrentImageUrl = () => {
    return getEventImageUrl(event.eventImageURL);
  };
  ...
  const handleImageError = () => {
    console.log('❌ Failed to load image:', getCurrentImageUrl());
    setImageError(true);
  };
      ...
      <View style={styles.imageSection}>
        {currentImageUrl && !imageError ? (
          <View style={styles.imagePreview}>
            <Image 
              source={{ uri: currentImageUrl }} // ✅ Using the calculated URL
              style={styles.image} 
        ...
        {image ? (
          ...
          <TouchableOpacity style={styles.imageUploadButton} onPress={selectImage}>
            <Text style={styles.imageUploadText}>
              {currentImageUrl && !imageError ? '+ Change Image' : '+ Select Banner Image'}
            </Text>
