...
const EcoMapScreen = ({ navigation }) => {
    ...
    const [stations, setStations] = useState([]); // NEW: Stations data
    ...
    useEffect(() => {
        ...
        fetchAllStations(); // Changed: Fetch stations instead of individual bins
    }, []);
    ...
    // NEW: Fetch all stations instead of individual bins
    ...
    // NEW: Fetch bins only for filtering
    ...
    // Apply filter to show individual bins
    const applyFilter = () => {
    ...
    const handleNavigateToStation = (station) => {
        console.log('Navigating to station:', station);

        setDestination(null);
        
        // Get bins for this station
        const fetchStationBins = async () => {
            try {
                ...
                const stationBins = response.data.map((bin, index) => ({
                    ...
                    location_description: station.description,
                    updated_at: bin.updated_at || new Date().toISOString(),
                    isStationBin: true,
                ...
                console.log('Fetched station bins:', stationBins);
                
                setSelectedStationBins(stationBins);
                
                if (stationBins.length > 0) {
                    setSelectedBin(stationBins[0]);
                }
                ...
                console.error('Error fetching station bins:', error);
